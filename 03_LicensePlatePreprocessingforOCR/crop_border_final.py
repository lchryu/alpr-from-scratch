...
def remove_special_chars(text: str) -> str:
    """
    Xóa TẤT CẢ ký tự special, chỉ giữ lại chữ số và chữ cái.
    Sau đó format lại theo đúng pattern biển số Việt Nam.
    """
    # Xóa TẤT CẢ ký tự không phải alphanumeric (bao gồm cả dấu chấm, gạch ngang)
...
def format_vietnamese_plate(text: str) -> str:
    """
    Format text thành đúng format biển số Việt Nam.
    Format: XX-XXXX.XX hoặc XX-XXXXX.XX
    """
    # Đảm bảo chỉ giữ lại alphanumeric (xóa lại một lần nữa để chắc chắn)
    ...
    if len(cleaned) == 8:
        ...
        # Format: XX-XXXXX.XX
        # Ví dụ: 61A22959 -> 61-A2295.9 (nếu 9 ký tự)
        return f"{cleaned[:2]}-{cleaned[2:7]}.{cleaned[7:]}"
    elif len(cleaned) == 7:
        # Có thể thiếu 1 số, thử format: XX-XXXX.X
        return f"{cleaned[:2]}-{cleaned[2:6]}.{cleaned[6:]}"
    else:
...
def validate_vietnamese_plate(text: str) -> bool:
    ...
    # Pattern 1: XX-XXXX.XX (8 ký tự: 2 số/chữ - 4 số/chữ - 2 số)
    # Pattern 2: XX-XXXXX.XX (9 ký tự: 2 số/chữ - 5 số/chữ - 2 số)
    # Cho phép chữ cái ở phần giữa (như 61A22959 -> 61-A229.59)
    patterns = [
...
if __name__ == "__main__":
    ...
    for idx, (plate_img, det_conf, bbox) in enumerate(plates, 1):
        ...
        # Bước 9: Xóa ký tự special và check valid
        # Chọn kết quả tốt hơn
        if conf_gray > conf_bin:
            best_text = text_gray
            best_conf = conf_gray
            best_method = "CleanGray"
        else:
            best_text = text_bin
            best_conf = conf_bin
            best_method = "Binary"
        
        # Xóa TẤT CẢ ký tự special (chỉ giữ alphanumeric) - BỎ HẾT DẤU CHẤM
        cleaned_text = remove_special_chars(best_text)
        
        # Format lại theo đúng pattern biển số Việt Nam
        formatted_text = format_vietnamese_plate(cleaned_text)
        ...
        print(f"  Best OCR       : '{best_text}' ({best_conf:.3f}) [{best_method}]")
        print(f"  Cleaned (raw)  : '{cleaned_text}' (len={len(cleaned_text)})")
        print(f"  Formatted      : '{formatted_text}'")
        print("")
        print(f"  {'='*20} RESULT {'='*20}")
        print(f"  Final Plate    : '{formatted_text}'")
        ...
        title_bin = f"9. Binary OCR\n'{formatted_text}'\nValid: {is_valid}"
        color_bin = "green" if is_valid else "red"
        axes[1, 4].set_title(title_bin, color=color_bin, fontweight="bold" if is_valid else "normal", fontsize=9)
        axes[1, 4].axis("off")
        ...
        title_gray = f"13. CleanGray OCR\n'{formatted_text}'\nValid: {is_valid}"
        color_gray = "green" if is_valid else "red"
        axes[2, 3].set_title(title_gray, color=color_gray, fontweight="bold" if is_valid else "normal", fontsize=9)
        axes[2, 3].axis("off")
